...
class semantic_estimator(torch.nn.Module):
    def __init__(self, feature_num, num_classes, ignore_label=255, resume=None):
        ...
        self.register_buffer('CoVariance', torch.zeros(self.class_num, feature_num))
    ...
    def init(self, resume=None):
        if resume is not None:
            ...
            self.CoVariance = checkpoint['state_dict']['student_estimator.CoVariance'].cuda(non_blocking=True)
            self.Mean = checkpoint['state_dict']['student_estimator.Mean'].cuda(non_blocking=True)
            self.Amount = checkpoint['state_dict']['student_estimator.Amount'].cuda(non_blocking=True)
